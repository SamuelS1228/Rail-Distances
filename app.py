...
def solve_one_pair(G, zip_to_nodes, edge_lookup, start_zip, end_zip):
    ...
    - rail_miles
    - num_segments
    - fraarcid_chain (ordered, separated by ' > ')
    - frnode_chain (ordered, separated by ' > ')
    """
...
if fra_file is not None and od_file is not None:
    ...
    if st.button("Run shortest path solver"):
        ...
        st.session_state["fra_df"] = fra_df
        st.session_state["start_col"] = start_col
        st.session_state["end_col"] = end_col

        # Full results download
        csv_buf_full = io.StringIO()
        results_df.to_csv(csv_buf_full, index=False)
        st.download_button(
            label="Download full results (all OD columns + path) as CSV",
            data=csv_buf_full.getvalue(),
            file_name="rail_shortest_paths_full.csv",
            mime="text/csv",
        )

        # Minimal OD/path export (distance + chains for each OD)
        export_cols = []
        for col in [start_col, end_col, "start_zip", "end_zip"]:
            if col in results_df.columns and col not in export_cols:
                export_cols.append(col)

        for col in ["rail_miles", "num_segments", "fraarcid_chain", "frnode_chain", "status", "message"]:
            if col in results_df.columns and col not in export_cols:
                export_cols.append(col)
        ...
        st.download_button(
            label="Download OD distance + chain export CSV",
            data=csv_buf_export.getvalue(),
            file_name="rail_od_paths_export.csv",
            mime="text/csv",
        )

    # Mapping section
    st.subheader("Map a solved OD pair")
    if "results_df" in st.session_state:
        results_df = st.session_state["results_df"]
        fra_df_cached = st.session_state["fra_df"]
        start_col_cached = st.session_state["start_col"]
        end_col_cached = st.session_state["end_col"]

        if len(results_df) > 0:
            def make_label(idx):
                row = results_df.loc[idx]
                status = row.get("status", "")
                s_zip = row.get("start_zip", row.get(start_col_cached, ""))
                e_zip = row.get("end_zip", row.get(end_col_cached, ""))
                miles = row.get("rail_miles", "")
                miles_str = f"{miles} mi" if pd.notna(miles) else ""
                return f"{idx}: {s_zip} → {e_zip} ({status}) {miles_str}"

            selected_idx = st.selectbox(
                "Select an OD row to map",
            ...
            if row.get("status") != "ok" or not isinstance(row.get("fraarcid_chain"), str):
                st.warning("Selected OD pair does not have a valid solved path.")
            else:
                chain_str = row["fraarcid_chain"]
                fra_ids = [c.strip() for c in chain_str.split(">") if c.strip() and c.strip() != "MISSING"]

                fra_segments = fra_df_cached[fra_df_cached["FRAARCID"].isin(fra_ids)].copy()

                if fra_segments.empty:
                    st.warning("Could not find FRA segments for this chain in the FRA file.")
                else:
                    # Ensure numeric
                    for col in ["Start_Node_Lat", "Start_Node_Long", "End_Node_Lat", "End_Node_Long"]:
                    ...
                    for seg in fra_segments.itertuples(index=False):
                        line_data.append({
                            ...
                            "from_lng": getattr(seg, "Start_Node_Long"),
                            "from_lat": getattr(seg, "Start_Node_Lat"),
                    ...
                    if line_df.empty:
                        ...
                        layer = pdk.Layer(
                            ...
                            get_color=[255, 0, 0],  # bright red for visibility
                            get_width=6,
                            pickable=True,
                        )
                        ...
                        st.pydeck_chart(
                            pdk.Deck(
                                ...
                                map_style="light",  # light basemap so the red line pops
                                tooltip={"text": "{fraarcid}"},
                            )
                        )
        else:
            st.info("Run the solver to see OD pairs you can map.")
    else:
